 ...
 * Return: timestamp if found, -ENOENT if bootstage id not found
 */
 ...
 * Return: 0 if exported ok, -EPERM if out of space
 */
 ...
 * Return: 0 if imported ok, -EPERM if out of space, -ENOENT if bootstage
 *	info not found, -ENOSPC if there is not space for read the imported
 *	data, or other error if something else went wrong
 */
